<main id="jazzDetail">
    ...
    <?php $fotocounter=0; ?>
    <?php $fotocounter++; ?>
    <img  src="/images/jazz/detail/<?= strtolower(str_replace(' ', '', $artist['name'])) ?>/<?=$fotocounter?>.png"
    ...
    <div class="container" style="display: flex; align-items: center;">
        ...
        <?php $fotocounter++; ?>
        <img src="/images/jazz/detail/<?= strtolower(str_replace(' ', '', $artist['name'])) ?>/<?=$fotocounter?>.png"
            alt="<?= htmlspecialchars($artist['name']) ?>" class="img-fluid mb-3" style="max-width: 40%;">
    </div>
        ...
        <div class="artistfoto-container" style="max-width: 50%; display: flex; gap: 10px;">
            ...
            <?php for ($i = 1; $i <= 2; $i++) { $fotocounter++; ?>
                <img src="/images/jazz/detail/<?= strtolower(str_replace(' ', '', $artist['name'])) ?>/<?=$fotocounter?>.png"
                    alt="<?= htmlspecialchars($artist['name']) ?>" class="img-fluid rounded">
                    <?php } ?>
            </div>
        </div>
        <!-- Player Container -->
        <div id="player" style="width: 50%; float: right;">
            <?php
            for ($i = 1; $i <= 3; $i++) {
                if (isset($artist["Song{$i}Link"])) {
                    $link = htmlspecialchars($artist["Song{$i}Link"]);
                    ...
                    elseif (strpos($link, 'soundcloud.com') !== false) {
                        echo "<iframe width='100%' height='166' scrolling='no' frameborder='no' allow='autoplay' 
